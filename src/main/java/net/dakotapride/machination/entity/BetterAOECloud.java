package net.dakotapride.machination.entity;
...
import net.dakotapride.machination.registrar.EntityRegistrar;
...
public class BetterAOECloud extends Entity implements TraceableEntity {
    private static final Logger LOGGER = LogUtils.getLogger();
    private static final EntityDataAccessor<Float> DATA_RADIUS = SynchedEntityData.defineId(BetterAOECloud.class, EntityDataSerializers.FLOAT);
    private static final EntityDataAccessor<Integer> DATA_COLOR = SynchedEntityData.defineId(BetterAOECloud.class, EntityDataSerializers.INT);
    private static final EntityDataAccessor<Boolean> DATA_WAITING = SynchedEntityData.defineId(BetterAOECloud.class, EntityDataSerializers.BOOLEAN);
    private static final EntityDataAccessor<ParticleOptions> DATA_PARTICLE = SynchedEntityData.defineId(BetterAOECloud.class, EntityDataSerializers.PARTICLE);
    ...
    private boolean canSetFire;
    private int fireSeconds;

    public BetterAOECloud(EntityType<? extends BetterAOECloud> entityType, Level level) {
        super(entityType, level);
        this.noPhysics = true;
    }

    public BetterAOECloud(Level level, double x, double y, double z) {
        this(EntityRegistrar.BETTER_AOE_CLOUD.get(), level);
        this.setPos(x, y, z);
    }
    ...
    public void setFixedColor(int fixedColor) {
        ...
        this.getEntityData().set(DATA_COLOR, fixedColor);
    }
    ...
    public void setCanSetFire(boolean canSetFire) {
        this.canSetFire = canSetFire;
    }
    ...
    public void setFireSeconds(int fireSeconds) {
    ...
    public void tick() {
        ...
        if (this.level().isClientSide) {
            ...
            if (this.tickCount % 5 == 0) {
                this.victims.entrySet().removeIf((integerEntry) -> this.tickCount >= integerEntry.getValue());
                List<MobEffectInstance> list = Lists.newArrayList();

                for(MobEffectInstance mobeffectinstance : this.potion.getEffects()) {
                    list.add(new MobEffectInstance(mobeffectinstance.getEffect(), mobeffectinstance.mapDuration((g) -> g / 4),
                    ...
                    if (!list1.isEmpty()) {
                        for(LivingEntity livingentity : list1) {
                            if (!this.victims.containsKey(livingentity) && livingentity.isAffectedByPotions()) {
                                ...
                                if (d3 <= (double)(f * f) && this.owner != livingentity) {
